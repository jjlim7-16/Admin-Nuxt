  ...
  <div class="column is-8">
    ...
    <div class="columns">
      <div class="column is-half">
        <b-field label='Select Station*' :type="errors.has('station') ? 'is-danger': ''" :message="errors.has('station') ? errors.first('station') : ''">
          <b-select expanded placeholder='Select Station' v-model="stationId" name="station" v-validate="'required'" data-vv-as="'Station'" rounded>
      ...
      <div class="column is-half">
        <b-field label='Select Role*' :type="errors.has('role') ? 'is-danger': ''" :message="errors.has('role') ? errors.first('role') : ''">
          <b-select expanded placeholder='Select Role' v-model="roleId" name="role" v-validate="'required'" data-vv-as="'Role'" @input="getSessionList" rounded>
    ...
    <b-field label="Remarks" :type="errors.has('remarks') ? 'is-danger': ''" :message="errors.has('remarks') ? errors.first('remarks') : ''">
      <b-input maxlength="500" type="textarea" name="remarks" v-validate.immediate="'required'" data-vv-as="'Remarks'" v-model="remarks"></b-input>
    </b-field>

    <br/>
    <button class="button is-success is-pulled-right" :disabled="isDisabled" @click="submit()">Save Changes</button>
    <router-link to="/Admin/Reservations/" class="button is-light is-pulled-right right-spaced">Cancel</router-link>
  </div>
</section>
</template>
