...
<section id="content" class="box columns is-multiline">
  <div class="column is-6">
    <b-field label='Station Name *' :type="errors.has('station') ? 'is-danger': ''" :message="errors.has('station') ? errors.first('station') : ''">
    ...
    <b-field label='Role Name *' :type="errors.has('roleName') ? 'is-danger': ''" :message="errors.has('roleName') ? errors.first('roleName') : ''">
      <b-input placeholder='Enter Role Name' name="roleName" v-model="roleName" data-vv-as="'Role Name'" v-validate="'required|alpha_spaces'" rounded>
      </b-input>
    </b-field>
  ...
  <div class="column is-9">
    <br>
    <!-- Add Role button -->
    <button id="addRoleBtn" class="button is-success is-pulled-right" :disabled="isDisabled" @click="submit()">Submit</button>
    <router-link to="/Admin/Roles/" class="button is-light is-pulled-right right-spaced">Cancel</router-link>
  </div>
</section>
</template>
  ...
  methods: {
    async submit() {
      ...
      if (this.roleList.find(i => i.role_name === this.roleName.trim())) {
        ...
        let stationName = this.stationList.find(i => i.station_id === parseInt(this.stationId)).station_name
