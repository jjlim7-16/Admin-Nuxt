...
export default {
	...
	data() {
		return {
			...
			sessionEndTime: "",
			stationName: "",
			stationID: this.$store.state.auth.user.station_id,
			numberOfBooking: 0,
			errMessage: "",
			filter: "",
			currentPage: 1,
	...
	created() {
		...
		// }
		console.log('Set stationid is ' + this.$store.state.auth.user.station_id);
		let theData;
		let startTime;
		let endTime;
		this.$axios.get(`http://${config.serverURL}/reservations/getCurrentReservation/${this.stationID}`)
			.then(res => {
				if (res.status === 200) {
		...
		this.$axios.get(`http://${config.serverURL}/bookings/getbookinglist/${this.stationID}`)
			.then(res => {
				if (res.status === 200) {
					console.log("bookinglist", res.data);
					if (res.data.length == 0) {
						this.$axios.get(`http://${config.serverURL}/sessions/currentSession/${this.stationID}`)
							.then(res => {
								if (res.status === 200) {
									if (res.data.length > 0) {
										console.log(res.data);
							...
							.catch(err => {
								console.log("FAIL at getting sessions");
								this.haveSession = false;
							});
					} else {
						theData = res.data;
			...
			.catch(err => {
				console.log("FAIL at second getting sessions");
				this.haveSession = false;
			});
	},
	mounted() {
								...
								if (res.data.length > 0) {
									for (var i of data) {
										...
										if (i.booking_status === "Confirmed") {
											...
											self.$dialog.alert({
												title: "Wrong Booking",
												message: "User does not have a booking here.<br/><br/>" +
